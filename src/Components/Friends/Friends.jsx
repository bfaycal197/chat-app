    ...
    <div className="friends-interface">
      <div className="friend-requests">
        ...
        <div className="friend-request-container">
          {friendRequests.map((request) => (
            <div key={request.id} className="friend-request" onClick={() => handleUserClick(request.id)}>
              ...
              {acceptedRequests.includes(request.id) ? (
                ...
                <div className='divbut'>
                  <button className="request-button" onClick={() => handleAccept(request.id)}>
                    Accept
                  </button>
                  <button className="request-button1" onClick={() => handleReject(request.id)}>
                    Reject
                  </button>
                </div>
              )}
      ...
      <div className="friends-list">
        ...
        <div className="friends-container">
          {friends.map((friend) => (
            <div key={friend.id} className="friend" onClick={() => handleUserClick(friend.id)}>
              <span>{friend.name}</span>
              {friend.deleted ? (
                <p>Deleted</p>
              ) : (
                <button className="request-button" onClick={() => handleDelete(friend.id)}>
                  Delete
                </button>
              )}
            </div>
